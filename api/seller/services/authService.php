...
<?php
    ...
    function login($POST){
        ...
        $query = "select * from users where (email='$email' or mobile='$email') and user_role=2 and user_type=1";
        ...
        if($query_res['status'] == 2){
            http_response_code(400);
            $array = array(
                'msg' => "You are Inactive, Wait for admin to approve your account.",
                'data' => $POST
            );
            echo json_encode($array);
            exit;
