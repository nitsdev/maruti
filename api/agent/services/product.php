<?php
    ...
    function addProduct($POST){
      ...
      if(count($_FILES['product_image']['name']))
      {
        ...
        $cols =  array("product_name", "short_description","description","length","width","height","weight","bullet1","bullet2","bullet3","stock","category_id","level1category_id","sub_category_id","product_owner","added_by","added_at","sku","status","price","discount","hsn","gst");
        $values =  array($product_name,$short_description,$description,$length,$width,$height,$weight,$bullet1,$bullet2,$bullet3,$stock,$category_id,$level1category_id,$sub_category_id,$GLOBALS['jwt_token']->user_id,$GLOBALS['jwt_token']->user_id,date('Y-m-d H:i:s'),$sku,0,$price,$discount,$hsn,$gst);
        $table_name = "products";
  ...
  function editProduct($POST){
    ...
    if($query_res){
      ...
      if(count($_FILES['product_image']['name']))
      {
        ...
        $updateQuery = "update products SET product_name='$product_name', short_description='$short_description', description='$description', price=$price, discount=$discount, length=$length, width=width, height=$height, weight=weight, bullet1='$bullet1', bullet2='$bullet2', bullet3='$bullet3', stock=$stock, category_id=$category_id, level1category_id=$level1category_id, sub_category_id=$sub_category_id, product_owner=$user_id, status='0', updated_by= $user_id, updated_at='$currdate',hsn='$hsn',gst=$gst where id = $id and product_owner=$user_id";
        update($updateQuery);  
