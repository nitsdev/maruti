<?php
    ...
    function register($POST){
      ...
      $query = "select * from users where (email='$email' or mobile='$mobile') and user_role=4";
      ...
      if(count($_FILES['agentImg']['name']))
      {
        for($imgCount=0; $imgCount<count($_FILES['agentImg']['name']);$imgCount++){
          $img = $_FILES['agentImg']['name'][$imgCount];
          $tmp = $_FILES['agentImg']['tmp_name'][$imgCount];
          // get uploaded file's extension
          $ext = strtolower(pathinfo($img, PATHINFO_EXTENSION));
          // can upload same image using rand function
          $final_image = rand(1000,1000000).time();
        ...
        $values =  array($POST['first_name'], $POST['last_name'], $POST['email'],$POST['mobile'],$hash_password,4,2,date('Y-m-d H:i:s'),$userType);
        $table_name = "users";
        ...
        $array = array(
            'msg' => "Please upload ID proof as well..",
            'data' => $_POST
        );
        http_response_code(401);
        echo json_encode($array);
