...
<html lang="en">
    <head>
        ...
        <title>Maruti Studio | Subscribed Users</title>
        <?php
            include('css.php')
        ?>
    </head>
    ...
    const token = localStorage.getItem("token");
        $.ajax({
            ...
            success: function(result) {
                ...
                for(let subUser = 0; subUser<data.length; subUser++){
                    const subUserHtml = `<tr>
                                            <td>${data[subUser].id}</td>
                                            <td>${data[subUser].subEmail}</td>
                                            <td><a href="mailto:${data[subUser].subEmail}">Send email</a></td>
                                        </tr>`; 
                    
                    $('#datatablesSimple').find('tbody').append(subUserHtml);
                }
            },
